...
func (a *Actuator) updateUserData(ctx context.Context, bmconfig *bmv1alpha1.BareMetalMachineProviderSpec,
	...
	// Get the worker machineconfigpool
	mcp := &mcfgv1.MachineConfigPool{}
	key = client.ObjectKey{
		Name: machine.Labels[machineRoleLabel],
	}
	err = a.client.Get(ctx, key, mcp)
	if err != nil {
		return fmt.Errorf("Cannot get \"%s\" machineconfigpool: %w", machine.Labels[machineRoleLabel], err)
	}
	...
	err = a.client.Get(ctx, key, mc)
	if err != nil {
		return fmt.Errorf("Cannot get the rendered machineconfig: %w", err)
	}
	...
	key = client.ObjectKey{
		...
		Namespace: bmconfig.UserData.Namespace,
	}
	...
	if err = json.Unmarshal(mc.Spec.Config.Raw, ignConfig); err != nil {
		return fmt.Errorf("cannot get unmarshal the machineconfig: %w", err)
	}

	// Merge the machineconfig ignition and the kubeconfig secret
	err = appendFileToIgnition(ignConfig, "/etc/kubernetes/kubeconfig", string(kubeconfigBytes))
	if err != nil {
		return fmt.Errorf("cannot append file to ignition: %w", err)
	}

	// Create a new secret with ignition data
	machineUserDataSecret = createNewMachineSecret(mc.Spec.Config.Raw, machine)
