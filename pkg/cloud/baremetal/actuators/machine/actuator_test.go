...
func TestUpdateUserData(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Scenario: "user data has both url and ca cert",
			IgnitionData: "{\"ignition\":{\"config\":{\"append\":[{\"source\":\"" +
				server.URL +
				"\",\"verification\":{}}]},\"security\":{\"tls\":{\"certificateAuthorities\":[{\"source\":\"data:text/plain;charset=utf-8;base64," +
				certB64 +
				"\",\"verification\":{}}]}},\"timeouts\":{},\"version\":\"2.2.0\"},\"networkd\":{},\"passwd\":{},\"storage\":{},\"systemd\":{}}",
			ExpectUpdate: true,
		},
		{
			Scenario: "user data has url but no ca cert",
			IgnitionData: "{\"ignition\":{\"config\":{\"append\":[{\"source\":\"" +
				server.URL +
				"\",\"verification\":{}}]},\"security\":{\"tls\":{\"certificateAuthorities\":[{\"source\":\"data:text/plain;charset=utf-8;base64," +
				"" +
	...
	for _, tc := range testCases {
		...
		// Create the pointer user data secret (worker-user-data)
		data := make(map[string][]byte)
		data["userData"] = []byte(base64.StdEncoding.EncodeToString([]byte(tc.IgnitionData)))
		...
		err = actuator.updateUserData(context.TODO(), config, &machine)
		if tc.ExpectUpdate && (config.UserData.Name != "machine1-user-data") {
			t.Errorf("expected config.UserData.Name %s, got %s", "machine1-user-data", config.UserData.Name)
		}
		if !tc.ExpectUpdate && (config.UserData.Name != testUserDataSecretName) {
			t.Errorf("expected config.UserData.Name %s, got %s", testUserDataSecretName, config.UserData.Name)
		}
	}
}
